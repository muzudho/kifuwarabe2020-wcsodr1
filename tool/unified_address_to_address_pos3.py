...
for file in range(1, 10):
    for rank in range(1, 10):
        print(
            f'                {enums[i]} => AddressPos3::FirstBoard(SquareType::Sq{file}{rank}),')
        i += 1
for file in range(1, 10):
    for rank in range(1, 10):
        print(
            f'                {enums[i]} => AddressPos3::SecondBoard(SquareType::Sq{file}{rank}),')
        i += 1
for phase in range(1, 3):
    for hand in ['King', 'Rook', 'Bishop', 'Gold', 'Silver', 'Knight', 'Lance', 'Pawn']:
        print(
            f'                {enums[i]} => AddressPos3::Hand(DoubleFacedPiece::{hand}{phase}),',)
        i += 1
print('        }')
print('    }')
print('}')
