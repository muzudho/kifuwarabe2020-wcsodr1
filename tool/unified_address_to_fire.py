...
for file in range(1, 10):
    for rank in range(1, 10):
        print(
            f'                {enums[i]} => Fire::new_board(Phase::First,AbsoluteAddress2D{{file:{file}, rank:{rank}}}),')
        i += 1
for file in range(1, 10):
    for rank in range(1, 10):
        print(
            f'                {enums[i]} => Fire::new_board(Phase::Second,AbsoluteAddress2D{{file:{file}, rank:{rank}}}),')
        i += 1
for hand in ['King', 'Rook', 'Bishop', 'Gold', 'Silver', 'Knight', 'Lance', 'Pawn']:
    print(
        f'                {enums[i]} => Fire::new_hand(Phase::First,DoubleFacedPieceType::{hand}),',)
    i += 1
for hand in ['King', 'Rook', 'Bishop', 'Gold', 'Silver', 'Knight', 'Lance', 'Pawn']:
    print(
        f'                {enums[i]} => Fire::new_hand(Phase::Second,DoubleFacedPieceType::{hand}),',)
    i += 1
print('        }')
print('    }')
print('}')
